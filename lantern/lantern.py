...
class AbstractAPI():
    ...
    def begin_prescan(self, app_id, build_id=None, auto_scan=False):
        '''
        Requires only app_id. build_id defaults to most recent. Returns the buildinfo xml.
        '''
        return self.request('get', '/4.0/beginprescan.do', {'app_id': app_id, 'build_id': build_id, 'auto_scan': auto_scan})
    ...
    def create_app(self, app_name, business_criticality, description=None, vendor_id=None, policy=None,
        ...
        '''
        Creates an application. Requires only app_name and business_criticality.
        Returns the appinfo xml.
        '''
    ...
    def get_call_stacks(self, build_id, flaw_id):
        '''
        Returns the callstacks_[app_id]_[build_id]_[flaw_id] XML document, which references the callstacks.xsd schema.
        '''
    ...
    def update_app(self, app_id, app_name=None, business_criticality=None,
        ...
        return self.request('get', '/4.0/updateapp.do',
            {'app_id': app_id, 'app_name': app_name, 'business_criticality': business_criticality, 'policy': policy,
             'business_unit': business_unit, 'business_owner': business_owner,
             'business_owner_email': business_owner_email, 'teams': teams, 'origin': origin, 'industry': industry,
             'app_type': app_type, 'deployment_type': deployment_type, 'archer_app_name': archer_app_name,
             'tags': tags, 'custom_field_name': custom_field_name, 'custom_field_value': custom_field_value})
    ...
    def update_build(self, app_id, build_id=None, version=None, lifecycle_stage=None, launch_date=None):
        ...
        return self.request('get', '/4.0/updatebuild.do',
            {'app_id': app_id, 'build_id': build_id, 'version': version, 'lifecycle_stage': lifecycle_stage,
...
class API(AbstractAPI):
    ...
    def get_prescan_results(self, tries=1, initial_delay=0, delay=1, backoff=2):
        success_list = ['Pre-Scan Success', 'Results Ready', 'Scan Internal Error', 'Scan In Process']
